...
dd1$habitat_pairs<-paste(dd1$Hab_Feed, dd1$Hab_T45)
...
```{r}
#paste to combine habitat types into one column
dd2$habitat_pairs<-paste(dd2$Hab_Feed, dd2$Hab_T60)
...
```{r}
#paste to combine habitat types into one column
dd3$habitat_pairs<-paste(dd3$Hab_Feed, dd3$Hab_T80)
...
```{r}
#paste to combine habitat types into one column
dd4$habitat_pairs<-paste(dd4$Hab_Feed, dd4$Hab_T125)
...
#colors: "Marble Blue","Greenish Blue", "Rosy Brown", "Velvet Maroon", "Orange Gold", "Red Fox", "Chestnut Red", "Firebrick", "Plum Pie", "Eggplant"
...
```
Modeling the effect of gut retention time, initial habitat location, and ending habitat location on the number of seed dispersal events within and across habitats.Does gut retention time influence dispersal from and to habitats?

```{r} 
#Models use the number of occurrences of dispersal events between and within each habitat as the explanatory variable.
#Models use a negative binomial distribution due to overdispresion (Zuur et al. 2009). Neg. binomial produced a better fitting model poisson.

m_intercept_nb<-glm.nb(Number~1, data = Habitatpair)

m1_nb<-glm.nb(Number~Initial_habitat+Dispersed_habitat, data = Habitatpair)
...
nb_tab #model 3 (GRT + Initial Habitat type is the best model)
