---
title: "Summary Stats Dispersal Distances 11Oct21"
output: html_document
author: "Andrea Blackburn"
#Added to GitHub 8Nov2021
---

```{r setup, include=FALSE}
#remove all N/A's prior to opening csv here
...
#### Mean, Variance, SD, & Maximum Dispersal Distances for each duration of gut transit time (45, 60, 80, 125-hours)
evidence of overdispersion

```{r}
#sample sizes for each duration of gut transit time
length(na.omit(dd$DD.45))
length(na.omit(dd$DD.60))
length(na.omit(dd$DD.80))
length(na.omit(dd$DD.125))

#45 hours Transit Time (TT) 
mean(dd$DD.45, na.rm=TRUE)
var(dd$DD.45, na.rm=TRUE)
max(dd$DD.45, na.rm=TRUE)
sd(dd$DD.45, na.rm=TRUE)

#60 hours TT
mean(dd$DD.60, na.rm=TRUE)
var(dd$DD.60, na.rm=TRUE)
max(dd$DD.60, na.rm=TRUE)
sd(dd$DD.60, na.rm=TRUE)

#80 hours TT
mean(dd$DD.80, na.rm=TRUE)
var(dd$DD.80, na.rm=TRUE)
max(dd$DD.80, na.rm=TRUE)
sd(dd$DD.80, na.rm=TRUE)

#125 hours TT
mean(dd$DD.125, na.rm=TRUE)
var(dd$DD.125, na.rm=TRUE)
max(dd$DD.125, na.rm=TRUE)
sd(dd$DD.125, na.rm=TRUE)
```
...
#ggsave("disp_density_gut_passage_rate.png", plot=plot_disp_density_gut_passage_rate, dpi=350, width =10, height =7)
```
...
dd125_100<-filter(dd4125, DD.125 >100)
31/31

```

##DISPERSAL DISTANCES BY SEX
...
#how many individuals per age-sex class
dd_null<-filter(dd, Age.sex == "Nulliparous Female")
length(unique(dd_null$Orangutan))

dd_par<-filter(dd, Age.sex == "Parous Female")
length(unique(dd_par$Orangutan))

dd_flm<-filter(dd, Age.sex == "Flanged Male")
length(unique(dd_flm$Orangutan))

dd_uflm<-filter(dd, Age.sex == "Unflanged Male")
length(unique(dd_uflm$Orangutan))
```

```{r}
#Pull out just the lost follows for each duration of gut passage time
#only needed for 45 and 80-hours GRT
...
#ggsave("Fruit available by dispersal distance 9Aug22.png", plot=plot_of_grids, dpi=350, width =14, height =10)
...
ggsave("Dispersal by Follow Duration and Age-Sex Class 45-hours 8Aug22.png", plot=g_lost45, dpi=350, width =9, height =7)


#60 hours DD
g8<-ggplot(dd2, aes(x=Lost_unable_to_follow, y=DD.60))+
  geom_boxplot()+ theme_classic()
g8
...
g_lost60 
ggsave("Dispersal by Follow Duration and Age-Sex Class 60-hours 8Aug22.png", plot=g_lost60 , dpi=350, width =9, height =7)

#Plot both graphs:
g_both<-plot_grid(g_lost45 , g_lost60 , nrow=1)
g_both
#ggsave("Dispersal by Follow Duration and Age-Sex Class 8Aug22.png", plot=g_both, dpi=350, width =15, height =8)
```
